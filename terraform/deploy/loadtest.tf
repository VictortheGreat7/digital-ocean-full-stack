...
resource "kubectl_manifest" "k6_loadtest" {
  yaml_body = <<-YAML
apiVersion: k6.io/v1alpha1
kind: K6
metadata:
...
spec:
  ...
  env:
    - name: BASE_URL
      value: "${var.subdomains[0]}.${var.domain}/api"
YAML
...
resource "kubectl_manifest" "k6_service_monitor" {
  yaml_body = <<-YAML
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
...
resource "kubectl_manifest" "k6_alert_rules" {
  yaml_body = <<-YAML
apiVersion: monitoring.coreos.com/v1
kind: PrometheusRule
metadata:
